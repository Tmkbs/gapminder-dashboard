...
# ------------------------------ CHART CREATION FUNCTIONS (STYLE FIX) ------------------
# 关键改动：我们不再使用 chart_template = "lux"
...
def create_population_chart(continent, year):
    ...
    # --- 移除 template="lux" ---
    fig = px.bar(df, x="Country", y="Population", color="Country", title=f"Top 15 Population in {continent} ({year})")
...
def create_gdp_chart(continent, year):
    ...
    # --- 移除 template="lux" ---
    fig = px.bar(df, x="Country", y="GDP per Capita", color="Country", title=f"Top 15 GDP Per Capita in {continent} ({year})")
    fig.update_layout(showlegend=False, title_x=0.5)
    return fig
...
def create_life_exp_chart(continent, year):
    ...
    # --- 移除 template="lux" ---
    fig = px.bar(df, x="Country", y="Life Expectancy", color="Country", title=f"Top 15 Life Expectancy in {continent} ({year})")
    fig.update_layout(showlegend=False, title_x=0.5)
    return fig
...
def create_choropleth_map(variable, year):
    ...
    # --- 移除 template="lux" ---
    fig = px.choropleth(df, color=variable, locations="ISO Alpha Country Code", locationmode="ISO-3", hover_data=["Country", variable], title=f"Global {variable} Map ({year})")
